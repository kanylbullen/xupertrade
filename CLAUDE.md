...
| Cloudflare API token | 40-char base64 | Phase (`TLS_CF_API_TOKEN`, read env-first), or Redis (`dashboard:tls:cf_token`) via operator-only `POST /api/tls/configure`. Either way it reaches Caddy only on the next `POST /api/tls/configure` |
...
- **Caddy admin API on `:2019` is open inside the Docker network.** Never publish it externally. The dashboard (`lib/caddy-admin.ts`) reaches it via `http://caddy:2019/load` for dynamic config push.
...
- **Caddy boots from its autosave, not the Caddyfile.** It runs with `--resume` (`docker-compose.yml`), so the last config pushed through `POST /api/tls/configure` survives restarts and recreates. `caddy/Caddyfile` applies only while `/config/caddy/autosave.json` doesn't exist, so editing the Caddyfile or `CADDY_HOST` does nothing on a configured host until you re-POST or remove the autosave. If Caddy crash-loops with `loading initial config: …` (for example the image lost a module the saved config uses), delete the autosave from a throwaway container and start it again. It then boots the self-signed bootstrap config: `docker run --rm -v "$(docker volume ls -q | grep caddy_config)":/config alpine rm /config/caddy/autosave.json && docker start hypertrade-caddy`.
