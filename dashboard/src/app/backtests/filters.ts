/**
 * Query-param parsing for the Backtests page, split out of page.tsx so
 * it can be unit-tested without rendering a server component.
 *
 * Mirrors app/trades/filters.ts (same URL grammar:
 * `?strategy=&from=&to=&page=`) but is deliberately a local copy, not a
 * shared import: the two pages evolve independently, and backtests may
 * grow period-based (`period_start`/`period_end`) filtering that the
 * trades page never needs.
 */
...
/**
 * The `to` filter is inclusive in the UI ("runs up to and including
 ...
 * and returns an empty table for a day that has runs.
 */
