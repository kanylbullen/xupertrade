...
async def test_redis_override_true_blocks_when_env_false(monkeypatch):
    ...
    monkeypatch.setattr(settings, "max_daily_loss_usd", 100)
    control = MagicMock()
    control.is_kill_switch_active = AsyncMock(return_value=True)
    pm = PortfolioManager(exchange=MagicMock(), control=control)
    assert await pm.check_risk_limits(is_open=True) is False
...
async def test_redis_override_false_allows_when_env_true(monkeypatch):
    ...
    monkeypatch.setattr(settings, "max_daily_loss_usd", 100)
    control = MagicMock()
    control.is_kill_switch_active = AsyncMock(return_value=False)
    pm = PortfolioManager(exchange=MagicMock(), control=control)
...
async def test_redis_read_failure_falls_back_to_env(monkeypatch):
    ...
    monkeypatch.setattr(settings, "max_daily_loss_usd", 100)
    control = MagicMock()
    control.is_kill_switch_active = AsyncMock(side_effect=RuntimeError("redis down"))
    pm = PortfolioManager(exchange=MagicMock(), control=control)
    # env=true → blocks
    assert await pm.check_risk_limits(is_open=True) is False
