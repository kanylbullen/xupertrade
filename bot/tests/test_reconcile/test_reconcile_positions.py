...
async def test_sell_fill_does_not_price_a_short_close(repo):
    ...
    trades = await _rows(repo, models.Trade)
    assert trades[0].reason == "reconcile: orphan close (estimated @ mid)"
...
async def test_two_rows_sharing_one_fill_both_get_a_trade_row(repo):
    """`Trade.order_id` is UNIQUE and one HL fill can close two rows.
    The second must fall back to a synthetic id instead of taking the
    whole transaction down with an IntegrityError."""
    ...
    ex = FakeExchange([[], []], fills=[_sell_fill(sz="2.0")])
