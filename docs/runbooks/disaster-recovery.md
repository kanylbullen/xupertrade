...
- En bot som inte är pausad kör reconcile när den startar och sedan var femte
  ...
  har bett om den. Sedan #186 håller första passet efter boot i stället alla
  sådana positioner och sätter reconcile hold (avsnitt 1.6). Först när en
  människa har rensat hold stänger pass 2 en ensam sådan position på ett mynt
  som boten handlar.
- Samma sak gäller en position som har bytt sida efter backupen. Pass 1 stänger
  DB-raden som `wrong-side`, och sedan saknar börspositionen rad.
...
inga order. Reconcile hold skyddar bara mot pass 2. **Kill switch skyddar inte
här.** Den blockerar bara nya öppningar, och reconcile går förbi den. Paus
stoppar inte allt (avsnitt 1.5).
...
När hold rensas körs ett reconcile-pass direkt, före alla öppningar. Det
marknadsstänger en ensam börsposition utan rad på ett mynt som boten handlar,
och sätter hold igen om flera finns kvar. En position på ett mynt som ingen
...
En bot utan sentinel startar med hold. Skriv inte in någon sentinel för hand
här. Hold är rätt läge tills steg 8.
...
|---|---|---|---|
| Position | Ingen rad | Första passet håller positionen och sätter hold om boten handlar myntet (avsnitt 1.6). När hold rensas marknadsstänger reconcile en ensam sådan position på ett mynt som boten handlar | Bestäm nu. Stäng den själv på HL (reduce-only), eller låt reconcile stänga den i steg 8 om den är ensam. Att skapa en DB-rad för hand avråds från, eftersom strategin saknar SL-state för positionen. |
| Position | Rad med motsatt sida | Radens strategi kan skicka en exit som förstorar positionen (avsnitt 1.4). Pass 1 stänger raden (`wrong-side`), och pass 2 håller sedan positionen som en position utan rad | Stäng av strategin (nedan). Bestäm om positionen som för en position utan rad. |
...
- Det första riktiga reconcile-passet körs vid första tick efter unpause,
  alltså inom ungefär en minut. Följ loggen tills det har gått. Det ska göra
  exakt det som tabellen i steg 7 förutsade, med en rad
  `Reconcile: closed orphan` eller `closed wrong-side` för varje sådan rad,
  och en rad `HELD exchange-orphan` för varje börsposition utan rad
  (avsnitt 1.6).
...
- Rensa hold först när varje hållen position är stängd för hand på HL, eller
  ...
  men bara om Redis tar emot skrivningar. API:t rensar även en hold som bara
  finns i botens minne. Rensningen kör ett reconcile-pass direkt (avsnitt
  1.6). Läs hold igen efteråt.
